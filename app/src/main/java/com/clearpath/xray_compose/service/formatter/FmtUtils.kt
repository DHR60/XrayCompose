...
object FmtUtils {
    ...
    private fun transportToQueryParams(item: ProfileModel): EncodedParameters {
        val params = EncodedParameters()
        val itemNetwork = item.network.ifEmpty { GlobalConst.defaultTransportNetworkEnum.name }
        val transportType =
            enumEntries<ETransport>().firstOrNull { it.name.equals(itemNetwork, ignoreCase = true) }
    ...
    private fun parseTransportParams(params: StringValues, baseItem: ProfileModel): ProfileModel {
        ...
        item = item.copy(network = transportEnum.name)
        when (transportEnum) {
            RAW -> {
