            ...
            item {
                FormCard {
                    Column(
                        ...
                        EditableTrailingIconField(
                            value = profileModel.port,
                            onValueChange = { newValue ->
                                if (newValue.isEmpty() || newValue.all { it.isDigit() }) {
                                    viewModel.updateProfileModel { currentState ->
                                        currentState.copy(port = newValue)
                                    }
                                }
                            },
                            label = { Text("Port") },
                            ...
                            onEditIconClick = {
                                activeDialogContext = FormBottomSheetContext(
                                    fieldKey = "port",
                                    title = "Edit Port",
                                    initialValue = profileModel.port,
                                    onConfirm = { newPort ->
                                        viewModel.updateProfileModel { currentState ->
                                            currentState.copy(port = newPort)
                                        }
                                    },
                ...
                FormCard {
                    Column(
                        ...
                        ExposedDropdownMenuBox(
                            ...
                            OutlinedTextField(
                                value = if (profileModel.network.isEmpty()) "Select Transport Methods"
                                else GlobalConst.transportReverseMap[enumValueOf<ETransport>(
                            ...
                            ExposedDropdownMenu(
                                ...
                                GlobalConst.transportMap.forEach { networkOption ->
                                    DropdownMenuItem(
                                        text = { Text(networkOption.key) },
                                        onClick = {
                                            viewModel.updateProfileModel { currentState ->
                                                currentState.copy(network = networkOption.value.toString())
                                            }
                                            expanded = false
                                        }
                                    )
                        ...
                        if (profileModel.network == ETransport.XHTTP.toString()) {
                            ...
                            ExposedDropdownMenuBox(
                                ...
                                OutlinedTextField(
                                    value = if (transportExtra.xhttpMode.isNullOrEmpty()) "Select XHTTP Mode" else transportExtra.xhttpMode!!,
                                    onValueChange = {},
                                    readOnly = true,
                                    ...
                                    colors = ExposedDropdownMenuDefaults.outlinedTextFieldColors(
                                        unfocusedTextColor = if (transportExtra.xhttpMode.isNullOrEmpty()) MaterialTheme.colorScheme.onSurfaceVariant else MaterialTheme.colorScheme.onSurface,
                                    )
                                )
                                ExposedDropdownMenu(
                                    expanded = xhttpExpanded,
                                    ...
                                    GlobalConst.xhttpTypeList.forEach { xhttpOption ->
                                        DropdownMenuItem(
                ...
                FormCard {
                    ...
                    ExposedDropdownMenuBox(
                        ...
                        ExposedDropdownMenu(
                            ...
                            GlobalConst.transportSecurityList.forEach { securityOption ->
                                DropdownMenuItem(
